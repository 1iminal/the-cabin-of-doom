...
public class Controller : MonoBehaviour
{
    ...
    public AudioClip clip_step_L;
    public AudioClip clip_step_R;
    private static int stepDelayWalk = 500;
    private static int stepDelayRun = 250;
    private bool foot = false;
    private float sinceLastStep = 0;
    ...
    void Update()
    {
        ...
        if (controller.state != GameState.PLAYING)
        {
            ...
            {
                SceneManager.LoadScene("EssentialsAudioScene");
            }
            return;
        } 
        Speed = 0;
        ...
        if (!LockControl)
        {
            ...
            if (move.sqrMagnitude > 0 && m_Grounded)
            {
                int stepDelay = running ? stepDelayRun : stepDelayWalk;
                sinceLastStep += Time.deltaTime*1000;
                if (sinceLastStep >= stepDelay || lastMove == 0)
                {
                    sinceLastStep = 0;
                    AudioClip clip = foot ? clip_step_L : clip_step_R;
